...
pub fn render_cloud_mode_loading_screen(
    ...
    let tip_element = if let Some(tip) = tip_model.as_ref(app).current_tip() {
        ...
        if let Some(link_target) = tip.link() {
            ...
            fragments.push(FormattedTextFragment::hyperlink(t!("ambient_agent.learn_more"), link_target));
        }
...
fn render_tier_limits_footer(
    ...
    fragments.push(FormattedTextFragment::hyperlink(t!("ambient_agent.upgrade"), upgrade_url));
    fragments.push(FormattedTextFragment::plain_text(
