...
impl CommentListView {
    ...
    fn menu_items_for_comment(
        ...
        let mut items = vec![MenuItemFields::new(t!("code_review.copy_text"))
            .with_icon(Icon::Copy)
            .with_on_select_action(CommentListAction::CopyCommentText)
            .into_item()];

        let mut edit_item = MenuItemFields::new(t!("code_review.edit"))
        ...
        if let Some(url) = html_url {
            items.push(
                MenuItemFields::new(t!("code_review.view_in_github"))
